...
export default function ChatTab({ onNavigate, initData }: ChatTabProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!fieldsResponse?.fields || fieldsResponse.fields.length === 0) {
        throw new Error(
          "No fields found on this page. Try refreshing the page or selecting a page with input fields.",
        );
      }
  ...
  const handleApplyChanges = async (messageIndex: number, changes: LLMResponse) => {
    try {
      ...
      setMessages((prev) =>
        prev.map((msg, idx) => (idx === messageIndex ? { ...msg, appliedChanges: true } : msg)),
      );
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-800">
                ...
                content={
                  <div className="space-y-1">
                    <p className="font-semibold">
                      Context Level determines what information the AI sees:
                    </p>
                    <p>
                      <strong>None:</strong> No context
                    </p>
                    <p>
                      <strong>Domain:</strong> Current website domain
                    </p>
                    <p>
                      <strong>URL:</strong> Full page URL
                    </p>
                    <p>
                      <strong>Selected Fields:</strong> Field labels and values
                    </p>
                    <p>
                      <strong>All Page:</strong> Entire page text content
                    </p>
                  </div>
                }
                side="right"
              ...
              {activeSkillsCount > 0 && (
                <Tooltip
                  content={
                    <div>
                      <p className="font-semibold">
                        {activeSkillsCount} active skill{activeSkillsCount !== 1 ? "s" : ""} on this
                        page
                      </p>
        ...
        {messages.length > 0 && (
          <Button variant="secondary" size="sm" onClick={handleClearChat} className="w-full">
            Clear Chat
          </Button>
        )}
      </div>
