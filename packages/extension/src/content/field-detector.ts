...
export class FieldDetector {
  ...
  private observeShadowRoot(shadowRoot: ShadowRoot) {
        ...
        if (mutation.type === "attributes" && mutation.target instanceof HTMLElement) {
          ...
          if (attributeName === "contenteditable") {
            ...
            if (isNowEditable && !wasKnown) {
              ...
              logger.debug(
                "[FieldDetector] Shadow DOM: element no longer contenteditable:",
                target,
              );
    ...
    observer.observe(shadowRoot, {
      ...
      subtree: true,
      attributes: true,
      attributeFilter: ["contenteditable", "disabled", "readonly", "role", "aria-disabled"],
    });
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && entry.target instanceof HTMLElement) {
            if (isEditableField(entry.target) && !this.knownFields.has(entry.target)) {
              logger.debug(
  ...
  private discoverNewFields() {
    ...
    traverseShadowDOM(
      ...
      (field) => {
        ...
        const shouldCheck = isContentEditable
          ? isEditableField(field, true)
    ...
    if (newCount > 0) {
      logger.debug(
        "[FieldDetector] Polling discovered",
        newCount,
        "new fields (including Shadow DOM)",
      );
    }
  }
