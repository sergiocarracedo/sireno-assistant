...
export default function FieldChangesMessage({ 
  ...
  const visibleChanges = showAll ? response.changes : response.changes.slice(0, INITIAL_VISIBLE_COUNT);
        ...
        <div className="flex justify-center">
          ...
          >
            {showAll ? (
              ...
              <>
                View {totalChanges - INITIAL_VISIBLE_COUNT} More {totalChanges - INITIAL_VISIBLE_COUNT === 1 ? 'Change' : 'Changes'} <ChevronDown className="h-3.5 w-3.5" />
              </>
            )}
          </button>
        </div>
