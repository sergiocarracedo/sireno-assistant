...
export default function SkillCard({ skill, isActive, isDisabled, onEdit, onDelete, onToggle }: SkillCardProps) {
  ...
  return (
    <Card className={`p-4 mb-3 transition-opacity ${isDisabled ? 'opacity-50' : ''} ${isActive && !isDisabled ? 'bg-green-50 border-green-500 dark:bg-green-950/30 dark:border-green-700' : ''}`}>
        ...
        <div className="flex items-start gap-3 flex-1">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-500">
              Domain: <code className="bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded text-xs">
                {domainMatch.pattern}
              </code>
      ...
      {intentTriggers && intentTriggers.length > 0 && (
        <div className="ml-12 mt-2 text-xs text-gray-600 dark:text-gray-400">
          Triggers: {intentTriggers.map((trigger: string, i: number) => (
